...
impl<'g> Grid<'g> {
    ...
    /// Return the number of grid cells (not edges).
    fn n_cells(&self) -> [usize; 4] {
        [
            self.edges[0].len() - 1,
            self.edges[1].len() - 1,
            self.edges[2].len() - 1,
    ...
    fn h(&self) -> &ArrayView1<'g, f64> {
        &self.edges[0]
    }

    fn k(&self) -> &ArrayView1<'g, f64> {
        &self.edges[1]
    }

    fn l(&self) -> &ArrayView1<'g, f64> {
        &self.edges[2]
    }

    fn kf(&self) -> &ArrayView1<'g, f64> {
        &self.edges[3]
    }
}

pub fn compute_single_crystal_norm(
    ...
    if n_traj == 0 {
        return Array4::zeros(grid.n_cells());
    };
    ...
    if n_threads == 1 {
        let mut out = Array4::<f64>::zeros(grid.n_cells());
        Zip::from(start.rows())
            .and(stop.rows())
            .and(&solid_angle)
