...
from scippneutron_algorithms.normalization import compute_single_crystal_norm
...
def test_single_crystal_norm_ins_det_traj_within_grid_2d(
    ...
    expected = helper.norm_grid(
        shape=tuple(len(e) - 1 for e in edges),  # type: ignore[arg-type]
        cells=cells,
        segments=segments,
        edges=edges,
    )
...
@pytest.mark.parametrize(
    ...
    ids=list(map(str, itertools.permutations(range(3), 3))),
)
...
def test_single_crystal_norm_ins_solid_angle_multiplies_norm(
    ...
    norm_1 = compute_single_crystal_norm(
        trajectory_start=trajectory_start,
        trajectory_stop=trajectory_stop,
        solid_angle=sc.array(dims=["pixel"], values=[1.0]),
        grid=edges,
    ...
    norm_2 = compute_single_crystal_norm(
        trajectory_start=trajectory_start,
        trajectory_stop=trajectory_stop,
...
def test_single_crystal_norm_ins_solid_angle_is_multiplied_per_detector(
    ...
    norm_combined = compute_single_crystal_norm(
        trajectory_start=trajectory_start,
        trajectory_stop=trajectory_stop,
